...
{
    public partial class KapakOpsiyonlariEkleForm : DevExpress.XtraBars.Ribbon.RibbonForm
    {
        IObjectSpace objectSpace;
        public KapakOpsiyonlariEkleForm(IObjectSpace objectSpace)
        {
            InitializeComponent();
        ...
        public void createTreeWithChecked()
        {
            ...
            foreach (Urunler satir in urunListeyukseklik)
            {
                if (satir.boyut != null)
                {
                    ...
                    {
                        if (satir2.yukseklik != null && satir.boyut != null)
                        {
        ...
        {
            int ctrl = 0;
            IList listaks = objectSpace.GetObjects(typeof(KapakOpsiyonlari));
            foreach (KapakOpsiyonlari satir in listaks)
            {
                if (satir.Opsiyon.Contains(textBox1.Text) || satir.OpsiyonKodu.Contains(textBox2.Text))
                {
                    ctrl = 1;
                    break;
                }
        ...
        private void bbiSaveAndClose_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            if (textBox1.Text != "")
            {
                if (textBox2.Text != "")
                {
                        ...
                        foreach (TreeNode node in checked_nodes)
                        {
                            ...
                            if (pathItems.Length == 5)
                            {
                                ...
                                foreach (Urunler satir in liste)
                                {
                                    ...
                                    ops.Opsiyon = textBox1.Text;
                                    ops.OpsiyonKodu = textBox2.Text;
                    ...
                    else { MessageBox.Show("Bu kapak opsiyonu zaten mevcut"); }
                }
                else { MessageBox.Show("Kapak opsiyonu kodu girmeniz gerekir"); }
            }
            else { MessageBox.Show("Kapak opsiyonu adı girmeniz gerekir"); }
        }
