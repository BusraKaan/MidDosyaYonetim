...
namespace MidDosyaYonetim.Module.BusinessObjects
{
    ...
    Criteria = "onay = true && red =false", Context = "ListView", BackColor = "#A5D6A7",
        FontColor = "Black", Priority = 1)]
    public class TeknikSartname : FileAttachmentBase, IObjectSpaceLink
    { // Inherit from a different class to provide a custom primary key, concurrency and deletion behavior, etc. (https://documentation.devexpress.com/eXpressAppFramework/CustomDocument113146.aspx).
        public TeknikSartname(Session session)
            : base(session)
        {
        }
        ...
        //        string queryString1 = string.Format("UPDATE [dbo].[TeknikSartname] SET OcrText = '" + text + "' WHERE [Oid] = '" + Oid + "'");
        ...
        private void AddOnay()
        {
            ...
            if (control == null)
            {
                ...
                {
                    if (satir.ObjectType == typeof(TeknikSartname))
                    {
                        allYetki.Add(satir);
                    }
                ...
                foreach (Yetkilendirme satir in allYetki)
                {
                    if (allYetki.Select(x => x.id).Contains(minid))
                    {
                        if (satir.id == minid)
                        {
                            ...
                            onaydoc.red = false;
                            onaydoc.File = File;
                            onaydoc.ObjectType = typeof(TeknikSartname);
                                ...
                                if (allYetki.Count > 1)
                                {
                                    foreach (Yetkilendirme satir2 in allYetki)
                                    {
                                        ...
                                        if (satir2.id == minid + 1)
                                        {
                                            ...
                                            onaydoc2.red = false;
                                            onaydoc2.File = File;
                                            onaydoc2.ObjectType = typeof(TeknikSartname);
        ...
        [XafDisplayName("Web'de Genel Teknik Şartnamelerde Göster")]
