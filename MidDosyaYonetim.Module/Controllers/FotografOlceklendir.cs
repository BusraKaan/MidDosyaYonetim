...
namespace MidDosyaYonetim.Module.Controllers
{
    ...
    public partial class FotografOlceklendir : ViewController
    {
        public FotografOlceklendir()
        {
            InitializeComponent();
            //TargetObjectType = typeof(Urunler);
        ...
        private void OlceklendirAction_Execute(object sender, SimpleActionExecuteEventArgs e)
        {
            //IList list = View.SelectedObjects;
            IObjectSpace objectSpace = Application.CreateObjectSpace();
            IList urunler = objectSpace.GetObjects(typeof(Urunler));

            foreach (Urunler item in urunler)
            {
                CriteriaOperator crtiteria = CriteriaOperator.Parse("urunler=?", item.Oid);
                ...
                if (fotograflar != null)
                {
                    foreach (Fotograflar foti in fotograflar)
                    {
                        ...
                        CriteriaOperator cr = CriteriaOperator.Parse("Urun=?", item.Oid);
